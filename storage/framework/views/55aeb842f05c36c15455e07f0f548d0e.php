...
<?php $component->withAttributes(['hide-nav' => true]); ?>
    ...
    <div x-data="{ tab: '<?php echo e($errors->any() || session('error') || session('success') ? 'input' : 'list'); ?>' }" class="min-h-screen bg-gray-50 pb-20 font-sans">
        ...
        <div class="bg-emerald-600 px-6 pt-8 pb-6 rounded-b-[30px] shadow-lg">
            ...
            <div class="bg-white/10 backdrop-blur-sm p-4 rounded-2xl mb-6 border border-white/20">
                ...
                <form action="<?php echo e(route('sekolah.petugas.sirkulasi.index')); ?>" method="GET">
                    <div class="relative">
        ...
        <div class="px-6 mt-4">
            <?php if(session('success')): ?>
                <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-xl relative mb-2 text-sm shadow-sm">
                    <strong class="font-bold">Sukses!</strong> <?php echo e(session('success')); ?>

                </div>
            <?php endif; ?>
            <?php if(session('error')): ?>
                <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-xl relative mb-2 text-sm shadow-sm">
                    <strong class="font-bold">Gagal!</strong> <?php echo e(session('error')); ?>
                    ...
                    <div class="flex justify-between items-start">
                        <div>
                            <h4 class="font-bold text-gray-800">
                                <?php echo e($item->santri->full_name ?? 'Nama Tidak Dikenal'); ?>

                            </h4>
                            <p class="text-xs text-gray-500 mt-1">
                                NIS: <span class="font-mono text-gray-700"><?php echo e($item->santri->nis ?? '-'); ?></span>
                            </p>
                            <p class="text-xs text-gray-500 mb-2">
                                Buku: <span class="text-emerald-600 font-medium"><?php echo e($item->buku->judul ?? 'Buku Terhapus'); ?></span>
                            </p>
                            <span class="text-[10px] bg-yellow-50 text-yellow-600 px-2 py-1 rounded-md border border-yellow-100">
                                Tenggat: <?php echo e(\Carbon\Carbon::parse($item->tgl_wajib_kembali)->format('d M Y')); ?>

                            </span>
                        </div>
                        
                        
                        <a href="<?php echo e(route('sekolah.petugas.sirkulasi.kembali.form', $item->id)); ?>" class="bg-emerald-50 text-emerald-600 px-3 py-1.5 rounded-lg text-xs font-bold border border-emerald-100 hover:bg-emerald-600 hover:text-white transition">
                            Kembalikan
                        </a>
                    </div>
                </div>
        ...
        <div x-show="tab === 'input'" style="display: none;" class="px-6 mt-6">
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                <form action="<?php echo e(route('sekolah.petugas.sirkulasi.store')); ?>" method="POST" class="space-y-4">
                    <?php echo csrf_field(); ?>
...
<?php endif; ?><?php /**PATH C:\laragon\www\keuangan-pesantren\resources\views/sekolah/petugas/sirkulasi/index.blade.php ENDPATH**/ ?>