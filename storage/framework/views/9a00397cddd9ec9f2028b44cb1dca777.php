        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-5 rounded-2xl shadow-sm border border-gray-200">
                <div>
                    ...
                    <a href="<?php echo e(route('sekolah.admin.monitoring.guru')); ?>" class="inline-flex items-center text-sm text-gray-500 hover:text-indigo-600 font-bold mb-2 transition group">
                    ...
                    <p class="text-sm text-gray-500 font-medium">
                        Periode: <span class="text-indigo-600 font-bold"><?php echo e(\Carbon\Carbon::create()->month($bulan)->locale('id')->isoFormat('MMMM')); ?> <?php echo e($tahun); ?></span>
                    </p>
                </div>
                ...
                <form method="GET" class="flex items-center gap-3 w-full md:w-auto">
                    <div class="relative flex-1 md:flex-none">
                        <select name="bulan" class="appearance-none bg-gray-50 border border-gray-300 text-gray-700 text-sm font-bold rounded-xl focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5 pr-8">
                            <?php for($i=1; $i<=12; $i++): ?>
                                <option value="<?php echo e($i); ?>" <?php if($i == $bulan): echo 'selected'; endif; ?>><?php echo e(\Carbon\Carbon::create()->month($i)->locale('id')->isoFormat('MMMM')); ?></option>
                            <?php endfor; ?>
                        </select>
                    ...
                    <div class="order-2 md:order-1 w-full md:w-1/3 bg-white rounded-2xl p-6 border-2 border-gray-300 shadow-sm relative flex flex-col items-center transform hover:-translate-y-1 transition">
                        ...
                        <div class="w-20 h-20 rounded-full border-4 border-gray-300 p-1 mb-3 mt-2">
                            <div class="w-full h-full rounded-full bg-gray-100 flex items-center justify-center text-xl font-bold text-gray-500">
                                <?php echo e(substr($laporan[1]->nama, 0, 2)); ?>

                            </div>
                        </div>
                        <h3 class="font-bold text-gray-800 text-center text-lg line-clamp-1 w-full"><?php echo e($laporan[1]->nama); ?></h3>
                        <p class="text-xs text-gray-500 mb-4 font-mono"><?php echo e($laporan[1]->nip ?? '-'); ?></p>
                        <div class="text-3xl font-black text-gray-700"><?php echo e($laporan[1]->skor); ?><span class="text-sm text-gray-400">%</span></div>
                    </div>
                    <?php endif; ?>

                    
                    <?php if(isset($laporan[0])): ?>
                    <div class="order-1 md:order-2 w-full md:w-1/3 bg-white rounded-2xl p-8 border-4 border-yellow-400 shadow-xl relative flex flex-col items-center rank-1-card">
                        ...
                        <div class="w-28 h-28 rounded-full border-4 border-yellow-400 p-1 mb-4 mt-2 shadow-sm bg-yellow-50">
                            <div class="w-full h-full rounded-full bg-white flex items-center justify-center text-3xl font-black text-yellow-600">
                                <?php echo e(substr($laporan[0]->nama, 0, 2)); ?>

                            </div>
                        </div>
                        <h3 class="font-black text-gray-900 text-center text-xl line-clamp-1 w-full"><?php echo e($laporan[0]->nama); ?></h3>
                        <p class="text-sm text-gray-500 mb-4 font-mono"><?php echo e($laporan[0]->nip ?? '-'); ?></p>
                        <div class="text-5xl font-black text-yellow-600"><?php echo e($laporan[0]->skor); ?><span class="text-xl text-gray-400 font-bold">%</span></div>
                        <p class="text-[10px] text-yellow-600 font-bold uppercase tracking-wide mt-1">Performa Terbaik</p>
                    </div>
                    <?php endif; ?>

                    
                    <?php if(isset($laporan[2])): ?>
                    <div class="order-3 md:order-3 w-full md:w-1/3 bg-white rounded-2xl p-6 border-2 border-orange-300 shadow-sm relative flex flex-col items-center transform hover:-translate-y-1 transition">
                        ...
                        <div class="w-20 h-20 rounded-full border-4 border-orange-300 p-1 mb-3 mt-2">
                            <div class="w-full h-full rounded-full bg-orange-50 flex items-center justify-center text-xl font-bold text-orange-500">
                                <?php echo e(substr($laporan[2]->nama, 0, 2)); ?>

                            </div>
                        </div>
                        <h3 class="font-bold text-gray-800 text-center text-lg line-clamp-1 w-full"><?php echo e($laporan[2]->nama); ?></h3>
                        <p class="text-xs text-gray-500 mb-4 font-mono"><?php echo e($laporan[2]->nip ?? '-'); ?></p>
                        <div class="text-3xl font-black text-gray-700"><?php echo e($laporan[2]->skor); ?><span class="text-sm text-gray-400">%</span></div>
                    </div>
                    <?php endif; ?>
                                ...
                                <?php $__currentLoopData = $laporan; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <tr class="hover:bg-indigo-50/30 transition-colors">
                                        <td class="px-6 py-4 whitespace-nowrap text-center">
                                            <span class="text-sm font-bold <?php echo e($index < 3 ? 'text-indigo-600' : 'text-gray-500'); ?>">
                                                <?php echo e($index + 1); ?>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center gap-3">
                                                <div class="w-9 h-9 rounded-lg bg-gray-100 text-gray-600 flex items-center justify-center font-bold text-xs border border-gray-200">
                                                    <?php echo e(substr($row->nama, 0, 2)); ?>

                                                </div>
                                                <div>
                                                    <p class="font-bold text-gray-800 text-sm"><?php echo e($row->nama); ?></p>
                                                    <p class="text-xs text-gray-500 font-mono"><?php echo e($row->nip ?? '-'); ?></p>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 align-middle">
                                            <div class="w-full max-w-xs mx-auto">
                                                ...
                                                <div class="progress-stack mb-1.5 shadow-inner">
                                                    ...
                                                    ?>
                                                    <div style="width: <?php echo e($pHadir); ?>%" class="bg-emerald-500" title="Hadir Tepat Waktu: <?php echo e($row->tepat_waktu); ?>"></div>
                                                    <div style="width: <?php echo e($pTelat); ?>%" class="bg-amber-400" title="Terlambat: <?php echo e($row->terlambat); ?>"></div>
                                                    <div style="width: <?php echo e($pIzin); ?>%" class="bg-blue-500" title="Izin/Sakit: <?php echo e($row->sakit + $row->izin); ?>"></div>
                                                </div>
                                                
                                                
                                                <div class="flex justify-between text-[10px] font-bold uppercase tracking-wide text-gray-400">
                                                    <span><span class="text-emerald-600"><?php echo e($row->tepat_waktu); ?></span> Tepat</span>
                                                    <span><span class="text-amber-600"><?php echo e($row->terlambat); ?></span> Telat</span>
                                                    <span><span class="text-blue-600"><?php echo e($row->sakit + $row->izin); ?></span> Izin</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right">
                                            ...
                                            ?>
                                            <span class="inline-flex items-center justify-center px-3 py-1 rounded-lg border font-black text-sm <?php echo e($scoreClass); ?>">
                                                <?php echo e($row->skor); ?>%
                                            </span>
                                        </td>
                                    </tr>
...
<?php endif; ?><?php /**PATH C:\laragon\www\keuangan-pesantren\resources\views/sekolah/admin/monitoring/kinerja-guru.blade.php ENDPATH**/ ?>