<x-app-layout hide-nav>
    ...
    <div class="min-h-screen bg-gray-50 pb-32">
        ...
        <div class="bg-emerald-600 px-5 pt-8 pb-24 shadow-md rounded-b-[30px] relative overflow-hidden">
            ...
            <div class="relative z-10 flex items-center gap-3">
                <a href="{{ route('orangtua.tagihan.index') }}" class="text-white hover:bg-emerald-700/50 p-2 rounded-full transition">
            ...
            <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                ...
                <div class="bg-gray-50/80 p-5 grid grid-cols-2 gap-y-4 gap-x-2 text-sm border-b border-gray-100">
                    <div>
                        ...
                        <p class="font-bold text-gray-800 truncate">{{ $tagihan->santri->full_name }}</p>
                    </div>
                    <div class="text-right">
                        <p class="text-gray-400 text-[10px] uppercase font-bold">Jatuh Tempo</p>
                        <p class="font-bold text-gray-800">{{ \Carbon\Carbon::parse($tagihan->due_date)->format('d M Y') }}</p>
                    </div>
                    <div class="col-span-2 pt-2 border-t border-gray-200/50">
                        ...
                        <p class="font-bold text-gray-800">{{ $tagihan->jenisPembayaran->name }}</p>
                ...
                {{-- Kalkulasi Total (Tanpa Rincian Item) --}}
                <div class="p-5">
                    <div class="space-y-2">
                        <div class="flex justify-between items-center text-sm">
                            <span class="text-gray-500">Subtotal</span>
                            <span class="font-medium">Rp {{ number_format($tagihan->nominal_asli, 0, ',', '.') }}</span>
                        </div>
                        @if($tagihan->nominal_keringanan > 0)
                            <div class="flex justify-between items-center text-sm text-green-600">
                                <span>Keringanan / Beasiswa</span>
                                <span>- Rp {{ number_format($tagihan->nominal_keringanan, 0, ',', '.') }}</span>
                            </div>
                        @endif
        ...
        <div class="px-5 mt-8">
            ...
            <div class="space-y-3">
                @if(isset($tagihan->transaksis) && $tagihan->transaksis->count() > 0)
                    @foreach($tagihan->transaksis as $trx)
                        <div class="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex justify-between items-center">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="text-[10px] text-gray-400 uppercase font-bold mb-0.5">{{ $trx->created_at->format('d M Y H:i') }}</p>
                                    <p class="text-xs font-bold text-gray-700 capitalize">{{ str_replace('_', ' ', $trx->metode_pembayaran) }}</p>
                                </div>
                            </div>
                            <div class="text-right">
                                <p class="text-sm font-bold text-emerald-600">+ {{ number_format($trx->total_bayar, 0, ',', '.') }}</p>
    ...
    {{-- FIXED BOTTOM BAR (ACTION BUTTON) --}}
    @if($totalSisa > 0)
        <div class="fixed bottom-0 left-0 w-full bg-white border-t border-gray-100 p-4 pb-6 shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)] z-50">
            <div class="max-w-3xl mx-auto flex items-center justify-between gap-4">
                <div class="hidden sm:block">
                    ...
                    <p class="text-lg font-bold text-gray-900">Rp {{ number_format($totalSisa, 0, ',', '.') }}</p>
                </div>
                <a href="{{ route('orangtua.tagihan.pilih-metode', $tagihan->id) }}" class="flex-1 sm:flex-none bg-emerald-600 text-white font-bold text-sm text-center py-3.5 px-6 rounded-xl shadow-lg shadow-emerald-200 hover:bg-emerald-700 transition active:scale-[0.98] flex items-center justify-center gap-2">
