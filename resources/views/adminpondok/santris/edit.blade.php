        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-xl border border-gray-100">
                ...
                <div class="px-8 py-5 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                    <div>
                        <h2 class="text-xl font-bold text-gray-800">Edit Data Santri</h2>
                        <p class="text-sm text-gray-500">Perbarui informasi untuk: <strong>{{ $santri->full_name }}</strong></p>
                    </div>
                    <div class="text-xs font-mono bg-gray-200 px-2 py-1 rounded text-gray-600">
                        NIS: {{ $santri->nis }}
                    </div>
                </div>

                <div class="p-8 text-gray-900">
                    <form method="POST" action="{{ route('adminpondok.santris.update', $santri->id) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            <div class="space-y-6">
                                ...
                                <div>
                                    ...
                                    <x-text-input id="nis" class="block mt-1 w-full" type="text" name="nis" :value="old('nis', $santri->nis)" required />
                                    <x-input-error :messages="$errors->get('nis')" class="mt-2" />
                                </div>

                                <div>
                                    <x-input-label for="full_name" :value="__('Nama Lengkap')" />
                                    <x-text-input id="full_name" class="block mt-1 w-full" type="text" name="full_name" :value="old('full_name', $santri->full_name)" required />
                                    <x-input-error :messages="$errors->get('full_name')" class="mt-2" />
                                </div>

                                <div>
                                    <x-input-label for="jenis_kelamin" :value="__('Jenis Kelamin')" />
                                    <select name="jenis_kelamin" id="jenis_kelamin" class="block mt-1 w-full border-gray-300 focus:border-emerald-500 focus:ring-emerald-500 rounded-md shadow-sm" required>
