        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-xl border border-gray-100">
                ...
                <div class="px-8 py-5 border-b border-gray-100 bg-gray-50">
                    <h2 class="text-xl font-bold text-gray-800">Edit Aturan Keringanan</h2>
                    <p class="text-sm text-gray-500">Perbarui nilai atau masa berlaku potongan.</p>
                </div>

                <div class="p-8 text-gray-900">
                    <form method="POST" action="{{ route('adminpondok.keringanans.update', $keringanan->id) }}">
                        @csrf
                        @method('PUT')
                                ...
                                <div>
                                    ...
                                    <select name="santri_id" id="santri_id" class="block mt-1 w-full border-gray-300 focus:border-emerald-500 focus:ring-emerald-500 rounded-md shadow-sm" required>
                                        ...
                                        @foreach ($santris as $santri)
                                            <option value="{{ $santri->id }}" {{ old('santri_id', $keringanan->santri_id) == $santri->id ? 'selected' : '' }}>
                                ...
                                <div>
                                    ...
                                    <select name="jenis_pembayaran_id" id="jenis_pembayaran_id" class="block mt-1 w-full border-gray-300 focus:border-emerald-500 focus:ring-emerald-500 rounded-md shadow-sm" required>
                                        ...
                                        @foreach ($jenisPembayarans as $jenis)
                                            <option value="{{ $jenis->id }}" {{ old('jenis_pembayaran_id', $keringanan->jenis_pembayaran_id) == $jenis->id ? 'selected' : '' }}>
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <select name="tipe_keringanan" id="tipe_keringanan" class="block mt-1 w-full border-gray-300 focus:border-emerald-500 focus:ring-emerald-500 rounded-md shadow-sm" required>
                                            <option value="persentase" {{ old('tipe_keringanan', $keringanan->tipe_keringanan) == 'persentase' ? 'selected' : '' }}>Persentase (%)</option>
                                            <option value="nominal_tetap" {{ old('tipe_keringanan', $keringanan->tipe_keringanan) == 'nominal_tetap' ? 'selected' : '' }}>Nominal Tetap (Rp)</option>
                                        </select>
                                    </div>
                                    <div>
                                        <x-input-label for="nilai" :value="__('Nilai Potongan')" />
                                        <x-text-input id="nilai" class="block mt-1 w-full" type="number" name="nilai" :value="old('nilai', $keringanan->nilai)" required />
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <x-text-input id="berlaku_mulai" class="block mt-1 w-full" type="date" name="berlaku_mulai" :value="old('berlaku_mulai', $keringanan->berlaku_mulai)" required />
                                    </div>
                                    <div>
                                        <x-input-label for="berlaku_sampai" :value="__('Sampai Tanggal')" />
                                        <x-text-input id="berlaku_sampai" class="block mt-1 w-full" type="date" name="berlaku_sampai" :value="old('berlaku_sampai', $keringanan->berlaku_sampai)" />
                                ...
                                <div>
                                    ...
                                    <x-text-input id="keterangan" class="block mt-1 w-full" type="text" name="keterangan" :value="old('keterangan', $keringanan->keterangan)" />
                                </div>
                            </div>
