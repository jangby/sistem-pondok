...
<body onload="window.print()">
    ...
    <div class="info">
        <table>
            ...
            <tr>
                <td>Santri</td>
                <td>: {{ $transaksi->calonSantri->nama_lengkap }}</td>
            </tr>
            <tr>
                <td>Metode</td>
                <td>: {{ strtoupper($transaksi->payment_type) }}</td>
            </tr>
        </table>
    </div>

    <div class="nominal">
        Rp {{ number_format($transaksi->gross_amount, 0, ',', '.') }}
    </div>

    <div style="text-align: right; font-size: 12px;">
        <p>Sisa Tagihan: Rp {{ number_format($transaksi->calonSantri->sisa_tagihan, 0, ',', '.') }}</p>
    </div>

    <div class="footer">
        <p>Terima kasih.<br>Simpan struk ini sebagai bukti pembayaran yang sah.</p>
        <p>Admin: {{ auth()->user()->name ?? 'Petugas' }}</p>
    </div>
