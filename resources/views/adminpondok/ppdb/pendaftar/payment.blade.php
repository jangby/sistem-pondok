<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Pembayaran PPDB') }} : {{ $calonSantri->nama_lengkap }}
            </h2>
            {{-- PERBAIKAN ROUTE: adminpondok. (tanpa underscore) --}}
            <a href="{{ route('adminpondok.ppdb.pendaftar.show', $calonSantri->id) }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg text-sm">
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {{-- Notifikasi Sukses + Tombol Cetak Auto --}}
            @if(session('print_url'))
                <div class="mb-6 bg-green-50 border border-green-200 text-green-800 px-4 py-3 rounded-lg flex justify-between items-center">
                    ...
                    <a href="{{ session('print_url') }}" target="_blank" class="bg-green-600 text-white px-4 py-2 rounded shadow hover:bg-green-700 flex items-center gap-2">
                ...
                <div class="md:col-span-1">
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-100 p-6 sticky top-6">
                        ...
                        <div class="bg-emerald-50 rounded-lg p-4 mb-6 border border-emerald-100">
                            ...
                            <p class="text-3xl font-bold text-emerald-800">Rp {{ number_format($calonSantri->sisa_tagihan, 0, ',', '.') }}</p>
                            <p class="text-xs text-gray-500 mt-1">Total Biaya: Rp {{ number_format($calonSantri->total_biaya, 0, ',', '.') }}</p>
                        </div>

                        @if($calonSantri->sisa_tagihan > 0)
                        {{-- PERBAIKAN ROUTE: adminpondok. (tanpa underscore) --}}
                        <form action="{{ route('adminpondok.ppdb.pendaftar.payment.store', $calonSantri->id) }}" method="POST">
                            @csrf
                            ...
                            <div class="mb-4">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" name="nominal" class="w-full pl-10 border-gray-300 rounded-lg focus:ring-emerald-500 focus:border-emerald-500" 
                                           placeholder="0" min="1000" max="{{ $calonSantri->sisa_tagihan }}" required>
                                </div>
                                <p class="text-xs text-gray-400 mt-1">*Maksimal Rp {{ number_format($calonSantri->sisa_tagihan, 0, ',', '.') }}</p>
                            </div>
                            ...
                            <table class="w-full text-sm text-left text-gray-500">
                                ...
                                <tbody>
                                    ...
                                    <tr class="bg-white border-b hover:bg-gray-50">
                                        ...
                                        <td class="px-4 py-3">
                                            ...
                                            @else
                                                <span class="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded border border-blue-200 uppercase">{{ str_replace('_', ' ', $trx->payment_type) }}</span>
                                            @endif
                                        </td>
                                        ...
                                        <td class="px-4 py-3 text-center">
                                            {{-- PERBAIKAN ROUTE: adminpondok. (tanpa underscore) --}}
                                            <a href="{{ route('adminpondok.ppdb.pendaftar.payment.print', $trx->id) }}" target="_blank" class="text-gray-500 hover:text-gray-800" title="Cetak Struk">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <tfoot class="bg-gray-50 border-t font-bold text-gray-700">
                                    <tr>
                                        ...
                                        <td class="px-4 py-3 text-right text-emerald-600">Rp {{ number_format($calonSantri->total_sudah_bayar, 0, ',', '.') }}</td>
                                        <td></td>
                                    </tr>
                                </tfoot>
                            </table>
