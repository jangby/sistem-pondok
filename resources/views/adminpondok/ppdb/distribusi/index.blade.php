    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                {{-- Filter Gelombang --}}
                <form action="{{ route('adminpondok.ppdb.distribusi.index') }}" method="GET" class="flex items-center gap-2 bg-white p-2 rounded-lg shadow-sm border border-gray-200">
                    ...
                    <select name="gelombang_id" onchange="this.form.submit()" class="border-none text-sm focus:ring-0 text-gray-700 font-medium bg-transparent cursor-pointer">
                        @foreach($allSettings as $st)
                            <option value="{{ $st->id }}" {{ $activeSetting->id == $st->id ? 'selected' : '' }}>
                                Tahun Ajaran {{ $st->tahun_ajaran }} ({{ $st->nama_gelombang }})
                            </option>
                        @endforeach
                    </select>
                </form>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow duration-300 relative group">
                        ...
                        <div class="p-6 border-b border-gray-50 flex justify-between items-start relative z-10">
                            <div class="flex items-center gap-4">
                                <div class="p-3 bg-{{ $color }}-50 text-{{ $color }}-600 rounded-lg">
                                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $style['icon'] }}"></path></svg>
                                </div>
                                <div>
                                    <h3 class="text-xl font-bold text-gray-800">{{ $data['label'] }}</h3>
                            ...
                            <div class="text-right">
                                ...
                                <span class="bg-{{ $color }}-100 text-{{ $color }}-700 text-lg font-bold px-3 py-1 rounded-md">
                        ...
                        <div class="p-6 grid grid-cols-2 gap-4 relative z-10 bg-white/80">
                            ...
                            <div>
                                <p class="text-xs text-gray-500 mb-1">
                                    {{ $key == 'panitia' ? 'Total Belanja' : 'Total Disetor' }}
                                </p>
                                <p class="text-base font-semibold text-gray-800 flex items-center gap-1">
                        ...
                        <div class="bg-gray-50 px-6 py-3 border-t border-gray-100 flex justify-between items-center relative z-10">
                            ...
                            <a href="{{ route('adminpondok.ppdb.distribusi.show', $key) }}?gelombang_id={{ $activeSetting->id }}" class="text-sm font-bold text-{{ $color }}-600 hover:text-{{ $color }}-800 flex items-center gap-1">
                                Lihat Rincian & Setor
                        ...
                        {{-- Dekorasi Background --}}
                        <div class="absolute top-0 right-0 w-32 h-32 bg-{{ $color }}-50 rounded-bl-full opacity-50 -mr-8 -mt-8 pointer-events-none group-hover:scale-110 transition-transform duration-500"></div>
                    </div>
                @endforeach
