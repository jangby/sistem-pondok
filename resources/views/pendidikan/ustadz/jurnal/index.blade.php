<x-ustadz-layout>
    ...
    <div class="py-4 px-4 max-w-md mx-auto">
        ...
        <div class="bg-gradient-to-r from-emerald-500 to-green-600 rounded-xl p-5 text-white shadow-lg mb-6 relative overflow-hidden">
            <div class="relative z-10">
                ...
                <div class="flex items-baseline mt-1">
                    <span class="text-4xl font-bold">{{ $totalHariIni }}</span>
        ...
        {{-- 2. Tombol Aksi Besar --}}
        <a href="{{ route('ustadz.jurnal.create') }}" class="block w-full bg-white border-2 border-dashed border-emerald-300 rounded-xl p-4 text-center hover:bg-emerald-50 hover:border-emerald-500 transition mb-6 group">
                ...
                <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex justify-between items-start">
                    <div>
                        <div class="flex items-center gap-2 mb-1">
                            <span class="font-bold text-gray-800">{{ $item->santri->full_name }}</span>
                    ...
                    <div class="text-right">
                        {{-- Badge Predikat --}}
                        @php
                            $color = match($item->predikat) {
                                'A', 'Lancar' => 'bg-green-100 text-green-700',
                                'B' => 'bg-blue-100 text-blue-700',
                                'C', 'Ulang' => 'bg-red-100 text-red-700',
                                default => 'bg-gray-100 text-gray-600'
                            };
                        @endphp
                        <span class="inline-block px-2 py-1 rounded-lg text-xs font-bold {{ $color }}">
                            {{ $item->predikat }}
                        </span>
