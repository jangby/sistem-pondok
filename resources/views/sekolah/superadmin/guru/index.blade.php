<x-app-layout>
        ...
        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div class="flex items-center gap-3">
                <span class="px-4 py-2 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-500 shadow-sm">
                    Total Guru: <span class="text-amber-600 font-black text-sm ml-1">{{ $users->total() }}</span>
                </span>
            </div>
        </div>
    </x-slot>
                                    ...
                                    <tr class="group hover:bg-amber-50/30 transition-colors">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <div class="text-xs text-gray-400">Terdaftar: {{ $user->created_at->format('d M Y') }}</div>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex flex-col gap-1">
                                                <div class="flex items-center gap-2 text-sm text-gray-600">
                                                    <span class="font-mono text-xs bg-gray-100 px-1.5 rounded border border-gray-200">{{ $user->guru->nip ?? '-' }}</span>
                                                </div>
                                                <div class="flex items-center gap-2 text-xs text-gray-500">
                                                    <svg class="w-3 h-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path></svg>
                                                    {{ $user->guru->telepon ?? $user->telepon ?? '-' }}
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right">
                                            <div class="flex items-center justify-end gap-2">
                                                ...
                                                <form action="{{ route('sekolah.superadmin.guru.destroy', $user->id) }}" method="POST" class="delete-form">
                                                    @csrf @method('DELETE')
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-40 overflow-y-auto p-2 border rounded-xl bg-gray-50">
                                @foreach($sekolahs as $sekolah)
                                    <label class="flex items-center p-2 rounded-lg hover:bg-white hover:shadow-sm cursor-pointer transition-all">
                                        ...
                                        <span class="ml-2 text-sm text-gray-700 font-medium">{{ $sekolah->nama_sekolah }}</span>
                                    </label>
                                @endforeach
                            </div>
    ...
    <script>
        ...
        const baseUpdateUrl = "{{ url('sekolah-superadmin/guru') }}"; 
        const storeUrl = "{{ route('sekolah.superadmin.guru.store') }}";
        ...
        @if($errors->any())
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.getElementById('nip').value = "{{ old('nip') }}";
                document.getElementById('telepon').value = "{{ old('telepon') }}";
                document.getElementById('alamat').value = "{{ old('alamat') }}";
                document.getElementById('rfid_uid').value = "{{ old('rfid_uid') }}";
                // Repopulate Checkboxes
                const oldSekolahIds = @json(old('sekolah_ids', []));
