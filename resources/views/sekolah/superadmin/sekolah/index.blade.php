<x-app-layout>
        ...
        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">
                    Kelola data sekolah formal (SMP & SMA) di lingkungan pondok.
                </p>
            </div>
            
            <div class="flex items-center gap-3">
                <span class="px-4 py-2 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-500 shadow-sm">
                    Total Unit: <span class="text-indigo-600 font-black text-sm ml-1">{{ $sekolahs->total() }}</span>
                </span>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-8 bg-gray-50/50 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if($sekolahs->count() > 0)
                        ...
                        <div class="group bg-white rounded-2xl p-6 shadow-sm border border-gray-200 hover:shadow-md hover:border-indigo-200 transition-all duration-200 relative overflow-hidden">
                            <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                                ...
                                <div class="flex items-center gap-5 w-full md:w-auto">
                                    <div class="flex-shrink-0 w-16 h-16 rounded-2xl {{ $badgeColor }} flex items-center justify-center text-2xl font-black border shadow-sm">
                                        {{ $initial }}
                                    </div>
                                    <div class="flex-1">
                                        <div class="flex flex-wrap items-center gap-3 mb-1">
                                            <h3 class="text-xl font-black text-gray-800 group-hover:text-indigo-600 transition-colors">{{ $sekolah->nama_sekolah }}</h3>
                                            <span class="px-2.5 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wide {{ $badgeColor }}">
                                                {{ $sekolah->tingkat }}
                                            </span>
                                        </div>
                                        <div class="space-y-1">
                                            @if($sekolah->kepala_sekolah)
                                                <p class="text-sm text-gray-600 flex items-center gap-1.5">
                                                    ...
                                                    <span class="font-semibold">Kepsek:</span> {{ $sekolah->kepala_sekolah }}
                                                </p>
                                            @endif
                                ...
                                <div class="flex items-center gap-6 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 pt-4 md:pt-0 border-gray-100">
                                    ...
                                    <div class="flex gap-6 text-center">
                                        <div>
                                            <div class="text-lg font-black text-gray-800">{{ $sekolah->guru_count }}</div>
                                            <div class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Guru</div>
                                        </div>
                                        <div class="w-px h-8 bg-gray-200"></div>
                                        <div>
                                            <div class="text-lg font-black text-gray-800">{{ $sekolah->siswa_count }}</div>
                                    ...
                                    <div class="flex items-center gap-2">
                                        {{-- PASSING DATA LENGKAP KE JS --}}
                                        <button onclick="openModal('edit', {{ $sekolah }})" 
                                        ...
                                        <form action="{{ route('sekolah.superadmin.sekolah.destroy', $sekolah->id) }}" method="POST" onsubmit="return confirm('Yakin hapus unit sekolah ini? Data guru & siswa terkait mungkin akan error.');">
                                            @csrf @method('DELETE')
                ...
                <div class="mt-6">{{ $sekolahs->links() }}</div>
            @else
            ...
            <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg opacity-0 scale-95" id="modalPanel">
                ...
                <form id="sekolahForm" method="POST">
                    ...
                    <div class="px-6 py-6 space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="col-span-2 md:col-span-1">
                                ...
                                <select name="tingkat" id="tingkat" required class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                                    ...
                                    {{-- Tambahkan opsi lain sesuai enum database --}}
                        ...
                        {{-- Kepala Sekolah (DITAMBAHKAN KEMBALI) --}}
                        <div>
    ...
    <script>
        ...
        const baseUpdateUrl = "{{ route('sekolah.superadmin.sekolah.index') }}";
        const storeUrl = "{{ route('sekolah.superadmin.sekolah.store') }}";
        ...
        @if($errors->any())
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.getElementById('nama_sekolah').value = "{{ old('nama_sekolah') }}";
                document.getElementById('tingkat').value = "{{ old('tingkat') }}";
                document.getElementById('kepala_sekolah').value = "{{ old('kepala_sekolah') }}";
