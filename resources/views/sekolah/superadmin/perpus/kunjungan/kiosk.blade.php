...
<div class="min-h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-6 relative overflow-hidden">
    ...
    <div class="w-full max-w-2xl bg-gray-800 p-8 rounded-2xl shadow-2xl border border-gray-700 z-10 relative">
        ...
        {{-- Form Input --}}
        <form action="{{ route('sekolah.superadmin.perpustakaan.kunjungan.store') }}" method="POST" class="mb-8">
            @csrf
            <div class="relative">
            ...
            {{-- Pilihan Keperluan (Opsional, default Membaca) --}}
                ...
                <div class="flex items-center justify-between bg-gray-700/50 p-3 rounded-lg">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 bg-blue-600 rounded-full flex items-center justify-center font-bold text-lg">
                            {{ substr($visit->santri->name, 0, 1) }}
                        </div>
                        <div>
                            <p class="font-bold text-white">{{ $visit->santri->name }}</p>
                            <p class="text-xs text-gray-400">{{ $visit->santri->kelas->nama_kelas ?? '-' }} • {{ $visit->waktu_berkunjung->format('H:i') }}</p>
                        </div>
                    </div>
                    <span class="text-xs bg-gray-600 px-2 py-1 rounded text-gray-300">{{ $visit->keperluan }}</span>
                </div>
                @endforeach
            </div>
        </div>
