        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                ...
                <div class="flex items-center gap-2 text-sm text-gray-500 mb-1">
                    <a href="{{ route('sekolah.admin.kegiatan-akademik.index') }}" class="hover:text-indigo-600 transition">Kegiatan</a>
                    <svg class="w-3 h-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                    <a href="{{ route('sekolah.admin.kegiatan-akademik.kelola.kelas', $kegiatan->id) }}" class="hover:text-indigo-600 transition">Daftar Kelas</a>
                    <svg class="w-3 h-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                    <span>Mapel</span>
                </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-indigo-100 p-6 flex flex-col md:flex-row justify-between items-center gap-6 relative overflow-hidden">
                ...
                <div class="relative z-10 flex items-center gap-5 w-full">
                    ...
                    <div class="flex-shrink-0 w-20 h-20 bg-indigo-600 rounded-2xl flex flex-col items-center justify-center text-white shadow-lg shadow-indigo-200">
                        ...
                        <span class="text-3xl font-black">{{ $kelas->nama_kelas }}</span>
                    </div>
                    
                    <div>
                        <h3 class="text-lg font-bold text-gray-900">{{ $kegiatan->nama_kegiatan }}</h3>
                        <div class="flex items-center gap-4 mt-1 text-sm text-gray-500">
                            ...
                            <span class="flex items-center gap-1">
                                ...
                                {{ $kelas->santris()->where('status', 'active')->count() }} Siswa
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex gap-3 w-full md:w-auto flex-shrink-0 relative z-10">
                    <div class="px-5 py-3 bg-gray-50 border border-gray-200 rounded-xl text-center">
                        ...
                        <div class="text-xl font-black text-gray-800">{{ $mapelList->count() }}</div>
                    </div>
                    <div class="px-5 py-3 bg-emerald-50 border border-emerald-100 rounded-xl text-center">
                        ...
                        <div class="text-xl font-black text-emerald-600">{{ $mapelList->where('completion', 100)->count() }}</div>
                    </div>
                </div>
            </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-100">
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            @forelse ($mapelList as $mapel)
                                ...
                                <tr class="group transition-colors duration-150 {{ $rowClass }}">
                                    {{-- Nama Mapel --}}
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 h-10 w-10 rounded-lg {{ $colorClass }} flex items-center justify-center font-bold text-xs border border-white shadow-sm uppercase">
                                                {{ $initials }}
                                            </div>
                                            <div class="ml-4">
                                                ...
                                                @if($mapel->kode_mapel)
                                                    <div class="text-xs text-gray-500 font-mono">{{ $mapel->kode_mapel }}</div>
                                                @endif
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 align-middle">
                                        <div class="w-full max-w-xs">
                                            ...
                                            <div class="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
                                                <div class="{{ $barColor }} h-2 rounded-full transition-all duration-1000" style="width: {{ $progress }}%"></div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end gap-2">
                                            {{-- Cetak (Dropdown Style / Grouped Icons) --}}
                                            ...
                                            {{-- Tombol Utama: Input Nilai --}}
                                            <a href="{{ route('sekolah.admin.kegiatan-akademik.kelola.nilai', ['kegiatan' => $kegiatan->id, 'kelas' => $kelas->id, 'mapel' => $mapel->id]) }}" 
