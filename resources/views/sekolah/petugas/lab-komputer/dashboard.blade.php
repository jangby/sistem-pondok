    ...
    <div class="relative pt-10 pb-16 px-6 rounded-b-[40px] overflow-hidden shadow-2xl bg-gradient-to-br from-blue-700 via-blue-600 to-indigo-900">
        ...
        <div class="relative z-10 flex justify-between items-center">
            <div class="text-white">
                ...
                <h1 class="text-3xl font-extrabold tracking-tight">Halo, {{ explode(' ', Auth::user()->name)[0] }} 👋</h1>
            </div>
        ...
        <div class="grid grid-cols-3 gap-3 mt-8 relative z-10">
            <div class="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/10 text-center shadow-lg">
                <span class="block text-3xl font-black text-white tracking-tighter">{{ $totalKomputer }}</span>
            ...
            <div class="bg-green-500/20 backdrop-blur-md rounded-2xl p-4 border border-green-400/30 text-center shadow-lg relative overflow-hidden">
                ...
                <span class="block text-3xl font-black text-green-300 tracking-tighter relative z-10">{{ $komputerOnline }}</span>
            ...
            <div class="bg-red-500/20 backdrop-blur-md rounded-2xl p-4 border border-red-400/30 text-center shadow-lg relative overflow-hidden">
                ...
                <span class="block text-3xl font-black text-red-300 tracking-tighter relative z-10">{{ $komputerOffline }}</span>
    ...
    <div class="px-6 -mt-8 relative z-20 pb-24">
        <div class="bg-white rounded-[30px] shadow-xl p-6 border border-gray-100">
            ...
            <div class="grid grid-cols-3 gap-y-8 gap-x-4">
                ...
                <a href="{{ route('petugas-lab.komputer.index') }}" class="flex flex-col items-center group cursor-pointer">
                ...
                <form action="{{ route('petugas-lab.shutdown.all') }}" method="POST" class="flex flex-col items-center w-full" onsubmit="return confirm('⚠️ KONFIRMASI:\n\nMatikan SEMUA komputer yang sedang Online?');">
                    @csrf
                ...
                <a href="{{ route('petugas-lab.refresh') }}" class="flex flex-col items-center group cursor-pointer">
                ...
                <a href="{{ route('petugas-lab.password.form') }}" class="flex flex-col items-center group cursor-pointer">
                ...
                <a href="{{ route('petugas-lab.jadwal') }}" class="flex flex-col items-center group cursor-pointer">
                ...
                <a href="{{ route('petugas-lab.laporan') }}" class="flex flex-col items-center group cursor-pointer">
        ...
        <div class="mt-6">
            ...
            <div class="space-y-3">
                ...
                <div class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex items-center transition-all hover:shadow-md">
                    <div class="relative">
                        <div class="w-10 h-10 rounded-xl {{ $pc->last_seen >= now()->subMinutes(2) ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400' }} flex items-center justify-center text-lg">
                            <i class="fas fa-desktop"></i>
                        </div>
                        @if($pc->last_seen >= now()->subMinutes(2))
                    ...
                    <div class="ml-4 flex-1">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-[10px] text-gray-400">{{ \Carbon\Carbon::parse($pc->last_seen)->diffForHumans(null, true) }}</span>
                        </div>
                        <div class="flex items-center mt-1">
                            @if($pc->pending_command)
                                <span class="px-2 py-0.5 bg-yellow-100 text-yellow-700 text-[10px] rounded font-bold mr-2">
                                    <i class="fas fa-hourglass-half mr-1"></i> {{ strtoupper($pc->pending_command) }}
                                </span>
                            @endif
                            <p class="text-xs text-gray-500 truncate max-w-[150px]">IP: {{ $pc->ip_address ?? '-' }}</p>
                        </div>
                    </div>
                </div>
                @empty
