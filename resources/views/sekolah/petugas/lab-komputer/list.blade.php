<x-app-layout hide-nav>
        ...
        <div class="max-w-3xl mx-auto px-4 mt-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4" id="pcListContainer">
                @forelse($computers as $pc)
                    ...
                    <div onclick="openModalDetail('{{ $pc->pc_name }}', '{{ $pc->password }}', '{{ $isOnline }}', '{{ $pc->ip_address }}')" 
                         class="pc-item relative bg-white p-5 rounded-2xl shadow-sm border {{ $cardBorder }} hover:shadow-md active:scale-[0.98] transition-all cursor-pointer group overflow-hidden">
                        
                        <div class="absolute left-0 top-0 bottom-0 w-1.5 {{ $statusColor }}"></div>

                        <div class="flex justify-between items-center pl-2">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-2xl {{ $isOnline ? 'bg-green-50 text-green-600' : 'bg-gray-50 text-gray-400' }} flex items-center justify-center text-xl shadow-inner">
                                ...
                                <div>
                                    <h3 class="pc-name text-lg font-bold text-gray-800 tracking-tight">{{ $pc->pc_name }}</h3>
                                    <div class="flex items-center gap-1.5 mt-0.5">
                                        <span class="relative flex h-2 w-2">
                                          <span class="animate-ping absolute inline-flex h-full w-full rounded-full {{ $statusColor }} opacity-75 {{ $isOnline ? '' : 'hidden' }}"></span>
                                          <span class="relative inline-flex rounded-full h-2 w-2 {{ $statusColor }}"></span>
                                        </span>
                                        <p class="text-xs font-semibold {{ $statusText }}">
    ...
    {{-- 3. MODAL DETAIL & PRINT (Premium UI) --}}
