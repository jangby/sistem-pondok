...
<head>
    ...
    <title>@yield('title', "Assa'adah Smart Digital") | Yayasan Pendidikan Modern</title>
    ...
    <nav id="navbar" class="fixed w-full z-50 transition-all duration-500 bg-white/90 backdrop-blur-xl border-b border-slate-200/50 shadow-sm">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-24 items-center">
                ...
                <div class="hidden md:flex space-x-10 items-center">
                    <a href="{{ route('landing.home') }}" class="text-sm font-bold {{ request()->routeIs('landing.home') ? 'text-emerald-600' : 'text-slate-600 hover:text-emerald-600' }} transition-colors">Beranda</a>
                    <a href="{{ route('landing.tentang') }}" class="text-sm font-bold {{ request()->routeIs('landing.tentang') ? 'text-emerald-600' : 'text-slate-600 hover:text-emerald-600' }} transition-colors">Tentang Kami</a>
                    <a href="{{ route('landing.program') }}" class="text-sm font-bold {{ request()->routeIs('landing.program') ? 'text-emerald-600' : 'text-slate-600 hover:text-emerald-600' }} transition-colors">Program</a>
                    <a href="{{ route('landing.ppdb') }}" class="text-sm font-bold {{ request()->routeIs('landing.ppdb') ? 'text-emerald-600' : 'text-slate-600 hover:text-emerald-600' }} transition-colors">Info PPDB</a>
                </div>

                <div class="hidden md:flex items-center gap-5">
                    @auth
                        <a href="{{ url('/dashboard') }}" class="px-6 py-2.5 text-sm font-bold text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-full transition">Dashboard</a>
                    @else
                        <a href="{{ route('login') }}" class="text-sm font-bold text-slate-600 hover:text-slate-900 transition">Portal Login</a>
                        <a href="{{ route('ppdb.register') }}" class="px-6 py-3 text-sm font-bold text-white bg-emerald-600 rounded-full hover:bg-emerald-700 hover:shadow-lg hover:shadow-emerald-500/30 transition-all transform hover:-translate-y-0.5">
                            Pendaftaran Santri
                        </a>
                    @endauth
                </div>
        ...
        <div id="mobile-menu" class="hidden md:hidden bg-white border-t border-slate-100 absolute w-full shadow-2xl">
            <div class="px-4 pt-4 pb-6 space-y-2">
                <a href="{{ route('landing.home') }}" class="block px-4 py-3 text-base font-bold rounded-xl hover:bg-emerald-50 text-slate-800">Beranda</a>
                <a href="{{ route('landing.tentang') }}" class="block px-4 py-3 text-base font-bold rounded-xl hover:bg-emerald-50 text-slate-800">Tentang Kami</a>
                <a href="{{ route('ppdb.register') }}" class="block mt-4 px-4 py-3 text-center text-base font-bold text-white bg-emerald-600 rounded-xl">Pendaftaran PPDB</a>
            </div>
        </div>
    </nav>
    ...
    <footer class="bg-slate-950 pt-20 pb-10 border-t border-slate-900 relative overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div class="border-t border-slate-800/80 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
                <p class="text-slate-500 text-sm font-medium">&copy; {{ date('Y') }} Yayasan Assa'adah. All rights reserved.</p>
