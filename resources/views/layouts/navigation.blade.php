...
<x-nav-link :href="route('superadmin.petugas-lab.index')" :active="request()->routeIs('superadmin.petugas-lab.*')">
    ...
    <x-dropdown align="left" width="48">
        ...
        <x-slot name="trigger">
            {{-- Pastikan variabel $ppdbClasses sudah didefinisikan di view/controller, 
                 atau ganti dengan class string biasa jika error --}}
            <button class="inline-flex items-center px-3 py-2 rounded-md text-sm font-medium leading-5 transition duration-150 ease-in-out {{ $ppdbClasses ?? 'text-gray-500 hover:text-gray-700 hover:border-gray-300 focus:outline-none focus:text-gray-700 focus:border-gray-300' }}">
                <div>PPDB Online</div>
                <div class="ms-1">
                    {{-- Icon Panah Bawah --}}
                    <svg class="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
