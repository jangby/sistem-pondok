    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                {{-- Card: Payout Tertunda (Warning/Action Needed) --}}
                    ...
                    <div class="relative z-10">
                        ...
                        <div class="mt-2 flex items-baseline gap-2">
                            <h3 class="text-3xl font-bold text-gray-900">
                                {{ $kpi_pending_payout_count }}
                            </h3>
                            <span class="text-sm text-orange-600 font-medium">Permintaan</span>
                        </div>
                        <p class="text-sm text-gray-500 mt-1">
                            Total: <span class="font-semibold text-orange-600">Rp {{ number_format($kpi_pending_payout_amount, 0, ',', '.') }}</span>
                        </p>
                    </div>
                    {{-- Icon Background --}}
                ...
                {{-- Card: Pemasukan Bulan Ini (Success/Growth) --}}
                    ...
                    <div class="relative z-10">
                        ...
                        <div class="mt-2">
                            <h3 class="text-3xl font-bold text-emerald-600">
                                Rp {{ number_format($kpi_revenue_this_month, 0, ',', '.') }}
                            </h3>
                        </div>
                    ...
                    <div class="relative z-10">
                        ...
                        <div class="mt-2">
                            <h3 class="text-2xl font-bold text-gray-900">
                                Rp {{ number_format($kpi_total_revenue, 0, ',', '.') }}
                            </h3>
                        </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border-l-4 border-indigo-500 p-6 relative overflow-hidden transition hover:shadow-md group">
                    <div class="relative z-10">
                        ...
                        <p class="text-sm text-gray-500 mt-1">
                            Melayani <strong class="text-indigo-600">{{ number_format($kpi_total_santri, 0, ',', '.') }}</strong> Santri
                        </p>
                    </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                {{-- KOLOM KIRI: Action List (Pending Payouts) --}}
                <div class="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                        ...
                        @forelse ($daftarPendingPayouts as $payout)
                            <div class="p-6 hover:bg-gray-50 transition duration-150 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                <div class="flex items-start gap-4">
                                    ...
                                    <div>
                                        <p class="text-lg font-bold text-gray-900">Rp {{ number_format($payout->total_amount, 0, ',', '.') }}</p>
                                        <p class="text-sm text-gray-600 font-medium">{{ $payout->pondok->name }}</p>
                                        <p class="text-xs text-gray-400 mt-1 flex items-center gap-1">
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                            Diajukan {{ $payout->requested_at->diffForHumans() }}
                                        </p>
                                    </div>
                                </div>
                                <div>
                                    <a href="{{ route('superadmin.payouts.show', $payout->id) }}" class="inline-flex items-center justify-center px-4 py-2 bg-emerald-600 border border-transparent rounded-lg font-semibold text-xs text-white uppercase tracking-widest hover:bg-emerald-700 focus:bg-emerald-700 active:bg-emerald-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 transition ease-in-out duration-150 w-full sm:w-auto">
                                        Proses
                                    </a>
                                </div>
                            </div>
                    ...
                    @if($kpi_pending_payout_count > 5)
                        <div class="bg-gray-50 p-4 text-center border-t border-gray-100">
                            <a href="{{ route('superadmin.payouts.index', ['status' => 'pending']) }}" class="text-sm font-medium text-emerald-600 hover:text-emerald-800 hover:underline">
