...
<head>
    <meta charset="UTF-8">
    <title>Rekap Buku Tamu - {{ $namaBulan }}</title>
    <style>
...
<body>
    ...
    <table class="kop-surat">
        <tr>
            ...
            <td width="90%" class="title-header">
                <h1>Buku Tamu Pondok Pesantren Assa'adah</h1>
        ...
        <tbody>
            @forelse($tamus as $index => $tamu)
                <tr>
                    ...
                    <td class="text-center">{{ \Carbon\Carbon::parse($tamu->jam_masuk)->locale('id')->isoFormat('D MMM Y') }}</td>
                    <td>
                        <span class="text-bold">{{ $tamu->nama_tamu }}</span><br>
                        <span style="font-size: 8pt; color: #555;">{{ $tamu->no_hp ? 'HP: '.$tamu->no_hp : '' }}</span>
                    </td>
                    <td>{{ $tamu->instansi_asal ?? '-' }}</td>
                    <td class="text-bold">{{ $tamu->bertemu_dengan }}</td>
                    <td>{{ $tamu->keperluan }}</td>
                    <td class="text-center">
                        <span class="waktu" style="color: #059669;">{{ \Carbon\Carbon::parse($tamu->jam_masuk)->format('H:i') }} WIB</span>
                        <br>s/d<br>
                        @if($tamu->jam_keluar)
                            <span class="waktu" style="color: #dc2626;">{{ \Carbon\Carbon::parse($tamu->jam_keluar)->format('H:i') }} WIB</span>
                        @else
    ...
    <div class="footer clearfix">
        <div class="ttd-container">
            <p style="margin-bottom: 5px;">Mengetahui,<br>{{ \Carbon\Carbon::now()->locale('id')->isoFormat('D MMMM Y') }}</p>
