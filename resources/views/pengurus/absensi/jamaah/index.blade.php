<x-app-layout hide-nav>
    ...
    <div class="min-h-screen bg-gray-50" 
        ...
        {{-- 2. INFO HAID (FLOATING) --}}
        <div class="px-5 -mt-16 relative z-20">
            <a href="{{ route('pengurus.absensi.jamaah.haid') }}" class="block bg-white rounded-3xl shadow-xl p-5 text-center border border-gray-100 backdrop-blur-xl active:scale-95 transition group">
                <div class="flex items-center justify-between">
                    <div class="text-left">
                        <p class="text-gray-400 text-[10px] font-bold uppercase tracking-widest">Santriwati Haid</p>
                        <h2 class="text-3xl font-black text-pink-500">{{ $sedangHaid }}</h2>
                    </div>
        ...
        <div class="px-5 mt-6 relative z-20">
             <a href="{{ route('pengurus.absensi.jamaah.scan') }}" class="flex items-center justify-center w-full bg-emerald-600 text-white font-bold py-4 rounded-2xl shadow-lg shadow-emerald-200 hover:bg-emerald-700 transition active:scale-95 group border border-emerald-500">
        ...
        <div class="px-5 mt-8">
            ...
            <div class="space-y-3">
                @foreach($sholatList as $sholat)
                    <div @click="openModal('{{ $sholat }}')" class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex justify-between items-center group active:scale-95 transition cursor-pointer hover:border-emerald-300">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <span class="font-bold text-gray-800 text-base">{{ $sholat }}</span>
                                <p class="text-[10px] text-gray-400">Klik untuk detail</p>
                            </div>
                        </div>
                        <div class="text-right">
                            <span class="block text-xl font-black text-emerald-600">{{ $dataSholat[$sholat]['total_hadir'] }}</span>
        ...
        {{-- 5. SPACER BLOCK (SOLUSI PERMANEN) --}}
        {{-- Ini adalah kotak kosong transparan setinggi 160px di paling bawah --}}
        {{-- Fungsinya memaksa halaman untuk bisa di-scroll lebih jauh ke bawah --}}
    ...
    <script>
        const sholatData = @json($dataSholat);
    </script>
