<x-app-layout hide-nav>
    ...
    <div class="min-h-screen bg-gray-900 flex flex-col p-5 font-sans relative overflow-hidden" x-data="scannerApp()">
        ...
        <div class="flex justify-between items-center mb-6 relative z-10">
            ...
            <a href="{{ route('pengurus.asrama.index') }}" class="bg-gray-800/50 backdrop-blur-sm px-4 py-2 rounded-xl text-gray-400 text-xs font-bold border border-gray-700 hover:bg-gray-800 hover:text-white transition">
                Selesai
            </a>
        </div>
        ...
        <div class="grid grid-cols-2 gap-3 mb-6 relative z-10">
            <a href="{{ route('pengurus.asrama.ketua.rekap') }}" class="bg-gray-800 p-3 rounded-2xl border border-gray-700 text-center active:scale-95 transition hover:border-purple-500 group">
                <span class="block text-white font-bold text-sm group-hover:text-purple-400">Laporan</span>
            </a>
            <a href="{{ route('pengurus.asrama.ketua.settings') }}" class="bg-gray-800 p-3 rounded-2xl border border-gray-700 text-center active:scale-95 transition hover:border-purple-500 group">
    ...
    {{-- Script Scanner (Sama seperti sebelumnya, arahkan ke route ketua) --}}
    <script>
        function scannerApp() {
            return {
                ...
                async processQueue() {
                    ...
                    try {
                        const response = await axios.post('{{ route('pengurus.asrama.ketua.process') }}', { rfid: item.code });
