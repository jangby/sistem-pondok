        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                    ...
                    <div>
                        ...
                        <div class="mt-2 text-gray-600 text-sm space-y-1">
                            <p class="flex items-center gap-2">
                                ...
                                <span class="font-semibold">Mulai:</span> {{ \Carbon\Carbon::parse($event->tanggal_mulai)->format('d M Y') }}
                            </p>
                            <p class="flex items-center gap-2">
                        ...
                        <div class="mt-3">
                            <span class="px-3 py-1 text-xs font-bold rounded-full {{ $event->is_active ? 'bg-emerald-100 text-emerald-800' : 'bg-red-100 text-red-800' }}">
                                {{ $event->is_active ? 'Status: Aktif' : 'Status: Selesai' }}
                            </span>
                        </div>
                    </div>
                    
                    <div class="flex flex-wrap gap-2 w-full md:w-auto">
                         <a href="{{ route('pengurus.perpulangan.index') }}" class="flex-1 md:flex-none justify-center px-4 py-2 bg-gray-500 text-white rounded-lg text-sm font-bold hover:bg-gray-600 flex items-center gap-2 transition">
                            &laquo; Kembali
                        </a>
                        <a href="{{ route('pengurus.perpulangan.scan') }}" class="flex-1 md:flex-none justify-center px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-bold hover:bg-blue-700 flex items-center gap-2 transition shadow-sm">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div class="bg-white p-4 rounded-xl shadow-sm border-t-4 border-gray-400 flex flex-col items-center justify-center text-center">
                    ...
                    <div class="text-3xl font-extrabold text-gray-800 mt-1">{{ $allRecords->count() }}</div>
                </div>
                ...
                <div class="bg-white p-4 rounded-xl shadow-sm border-t-4 border-blue-400 flex flex-col items-center justify-center text-center">
                    ...
                    <div class="text-3xl font-extrabold text-blue-800 mt-1">
                        {{ $allRecords->where('status', 1)->count() }}
                    </div>
                ...
                <div class="bg-white p-4 rounded-xl shadow-sm border-t-4 border-emerald-400 flex flex-col items-center justify-center text-center">
                    ...
                    <div class="text-3xl font-extrabold text-emerald-800 mt-1">
                        {{ $allRecords->where('status', 2)->count() }}
                    </div>
                </div>
                ...
                <div class="bg-white p-4 rounded-xl shadow-sm border-t-4 border-red-400 flex flex-col items-center justify-center text-center">
                    ...
                    <div class="text-3xl font-extrabold text-red-800 mt-1">
                        {{ $allRecords->filter(function($r) use ($batasAkhir, $sekarang) {
                ...
                <div class="p-4 border-b border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4 bg-gray-50">
                    <form action="" method="GET" class="flex flex-col md:flex-row items-center gap-2 w-full md:w-auto">
                        <div class="relative w-full md:w-64">
                            ...
                            <input type="text" name="search" placeholder="Cari Nama/NISM..." value="{{ request('search') }}" class="pl-10 w-full rounded-lg border-gray-300 text-sm focus:ring-emerald-500 focus:border-emerald-500">
                        </div>
                    ...
                    <table class="w-full text-sm text-left text-gray-500">
                        ...
                        <tbody>
                            ...
                            <tr class="bg-white border-b hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4">
                                    <div class="font-bold text-gray-900">{{ $record->santri->full_name }}</div>
                                    <div class="text-xs text-gray-500">{{ $record->santri->nis }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $record->santri->kelas->nama_kelas ?? '-' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($record->status == 2)
                                ...
                                <td class="px-6 py-4 text-center whitespace-nowrap">
                                    <a href="{{ route('pengurus.perpulangan.edit', $record->id) }}" class="text-blue-600 hover:text-blue-900 text-xs font-bold border border-blue-200 px-3 py-1 rounded hover:bg-blue-50 transition">
                                        Detail
                                    </a>
                                </td>
                            </tr>
