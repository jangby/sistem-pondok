...
class RaporController extends Controller
{
    ...
    public function generate(Request $request)
    {
        ...
        foreach ($santris as $santri) {
            ...
            // --- 2. LOGIKA NILAI (PERBAIKAN DISINI) ---

            // A. Rata-rata Akademik (IPK)
            $avgAkademik = NilaiPesantren::where('santri_id', $santri->id)
                ->where('mustawa_id', $request->mustawa_id)
                ->avg('nilai_akhir') ?? 0;

            // B. Nilai Kehadiran (FIX: Ambil MAX, bukan AVG)
            // Kita cari satu nilai kehadiran tertinggi dari pelajaran apapun yang sudah diinput.
            // Ini menghindari nilai anjlok karena ada pelajaran yang belum diinput (nilai 0).
            $nilaiKehadiranRaw = NilaiPesantren::where('santri_id', $santri->id)
                ->where('mustawa_id', $request->mustawa_id)
                ->where('nilai_kehadiran', '>', 0) // Hanya ambil yang ada isinya
                ->max('nilai_kehadiran'); 
            
            $nilaiKehadiranFinal = $nilaiKehadiranRaw ? round($nilaiKehadiranRaw) : 0;
            
            // C. Nilai Sikap
            // Rumus tetap (60% Kehadiran + 40% Akademik), tapi karena nilai kehadiran sudah diperbaiki (tinggi),
            // maka nilai sikap otomatis akan ikut tinggi/normal.
